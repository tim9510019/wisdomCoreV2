...
def transplant_and_freeze(model_id, agiv2_base):
    ...
    for i in range(32):
        ...
        if is_global:
            new_sd[f"{a_pre}norm2.weight"] = src_sd.get(
                f"{s_pre}input_layernorm.weight"
            )
            new_sd[f"{a_pre}norm3.weight"] = src_sd.get(
                f"{s_pre}post_attention_layernorm.weight"
            )
        else:
            new_sd[f"{a_pre}norm1.weight"] = src_sd.get(
                f"{s_pre}input_layernorm.weight"
            )
            new_sd[f"{a_pre}norm2.weight"] = src_sd.get(
                f"{s_pre}post_attention_layernorm.weight"
    ...
    print(f"✅ 知識庫移植完成！")
    print(
        f"🔒 凍結 Llama-3 參數: {frozen_count:,} | 🟢 新架構可訓練參數: {trainable_count:,}"
    )
...
class Packed32KDataset(torch.utils.data.IterableDataset):
    def __init__(self, tokenizer, max_length=32768, is_val=False, max_samples=None):
        ...
        ds_cos = load_dataset(
            "HuggingFaceTB/cosmopedia", name="stanford", split="train", streaming=False
        )
