...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    hidden_states = hidden_states[:, :, :, None, :, :].expand(
        B, N, num_kv_heads, n_rep, seq_len, head_dim
    )
...
class AGIV2LocalBlock(nn.Module):
    ...
    def forward(self, X, shift_size=0):
        ...
        Q_loc = (
            self.W_q_loc(Z_chunked)
            .view(B, N, self.C, self.num_heads, self.head_dim)
            .transpose(2, 3)
        )
        K_loc = (
            self.W_k_loc(Z_chunked)
            .view(B, N, self.C, self.num_kv_heads, self.head_dim)
            .transpose(2, 3)
        )
        V_loc = (
            self.W_v_loc(Z_chunked)
            .view(B, N, self.C, self.num_kv_heads, self.head_dim)
            .transpose(2, 3)
        )
...
class AGIV2GlobalBlock(nn.Module):
    ...
    def forward(self, X, shift_size=0):
        ...
        K_cross = repeat_kv(K_cross.unsqueeze(1), self.num_key_value_groups).squeeze(
            1
        )  # [B*N, nh, M, hd]
        ...
        V_cross = repeat_kv(V_cross.unsqueeze(1), self.num_key_value_groups).squeeze(
            1
        )  # [B*N, nh, M, hd]
