...
class AGIV2GlobalBlockFast(nn.Module):
    def __init__(self, D=D_MODEL, hidden_dim=6912, K=1024, M=1024, C=1024, num_heads=4, num_kv_heads=1, head_dim=256, rope_base=1000000.0):
        super().__init__()
        self.D = D
        self.C = C
        self.M = M
    ...
    def fuse_qkv(self):
        ...
        if hasattr(self, 'W_q_loc') and self.W_q_loc is not None:
            self.W_qkv_loc = nn.Linear(self.D, self.W_q_loc.out_features + self.W_k_loc.out_features + self.W_v_loc.out_features, bias=False).to(self.W_q_loc.weight.device).to(self.W_q_loc.weight.dtype)
            self.W_qkv_loc.weight.data = torch.cat([self.W_q_loc.weight.data, self.W_k_loc.weight.data, self.W_v_loc.weight.data], dim=0)
        ...
        if hasattr(self, 'W_k_mem') and self.W_k_mem is not None:
            self.W_kv_mem = nn.Linear(self.D, self.W_k_mem.out_features + self.W_v_mem.out_features, bias=False).to(self.W_k_mem.weight.device).to(self.W_k_mem.weight.dtype)
            self.W_kv_mem.weight.data = torch.cat([self.W_k_mem.weight.data, self.W_v_mem.weight.data], dim=0)
        ...
        if hasattr(self, 'W_k_cross') and self.W_k_cross is not None:
            self.W_kv_cross = nn.Linear(self.D, self.W_k_cross.out_features + self.W_v_cross.out_features, bias=False).to(self.W_k_cross.weight.device).to(self.W_k_cross.weight.dtype)
            self.W_kv_cross.weight.data = torch.cat([self.W_k_cross.weight.data, self.W_v_cross.weight.data], dim=0)
    ...
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        if hasattr(self, 'W_kv_mem'):
            KV_mem = self.W_kv_mem(normed_X_mem).view(B, L, 2, NUM_POOL_HEADS, POOL_HEAD_DIM)
        ...
        Q_mem_exp = self.Q_mem.unsqueeze(0).expand(B, -1, -1, -1) # Shape: (B, M, 9, 128)
        ...
        if hasattr(self, 'W_qkv_loc'):
            ...
            K_loc = QKV_loc[..., q_dim : q_dim + kv_dim].view(B, L, self.num_kv_heads, self.head_dim)
        ...
        # Local 分支
        attn_out_loc = flash_attn_func(Q_loc, K_loc, V_loc, dropout_p=0.0, causal=True, window_size=(self.C - 1, 0))
        ...
        if hasattr(self, 'W_kv_cross'):
            KV_cross = self.W_kv_cross(M_global).view(B, self.M, 2, self.num_kv_heads, self.head_dim)
            K_cross, V_cross = KV_cross.unbind(dim=2)
        else:
            K_cross = self.W_k_cross(M_global).view(B, self.M, self.num_kv_heads, self.head_dim)
            V_cross = self.W_v_cross(M_global).view(B, self.M, self.num_kv_heads, self.head_dim)
        ...
        I_cross_fa = flash_attn_func(Q_cross, K_cross, V_cross, dropout_p=0.0, causal=False)
        I_cross = self.o_proj_cross(I_cross_fa.reshape(B, L, self.num_heads * self.head_dim))
...
class AGIV2GFast(nn.Module):
    def __init__(self, vocab_size=262144, D=D_MODEL, hidden_dim=6912, num_blocks=26, C=1024, K=1024, M=1024, num_heads=4, num_kv_heads=1, head_dim=256, rope_local=10000.0, rope_global=1000000.0):
        super().__init__()
        self.D = D
        ...
        for i in range(num_blocks):
            if (i + 1) % 6 == 0:
                self.blocks.append(AGIV2GlobalBlockFast(D=D, hidden_dim=hidden_dim, K=K, M=M, C=C, num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim, rope_base=rope_global))
            else:
