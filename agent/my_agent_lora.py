...
class MyAgent(Agent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.control_colors = {1, 9}  # Persistent control colors for the level (1 and 9 are controls in this game)
    ...
    def choose_action(self, frames, latest_frame) -> GameAction:
        try:
            ...
            if self.action_history and self.action_history[-1]["outcome"] == "No change.":
                if self.action_queue:
                    print(f"[{self.game_id}] Last action resulted in 'No change.'. Clearing remaining {len(self.action_queue)} queued actions.")
                    self.action_queue.clear()

            # 2. Check if new level is reached (score changed)
            ...
            if current_level != self.current_score:
                ...
                # [一般化解法 - 對策A] 關卡升級時，重置 LoRA 權重回初始狀態，防止跨關卡負遷移與干擾
                if self.current_score != -1 and current_level > self.current_score:
                    initial_state_key = "initial_lora_state_google/gemma-4-E2B-it"
                    if initial_state_key in _MODEL_CACHE:
                        print(f"[LoRA Reset] Level upgraded from {self.current_score} to {current_level}. Resetting LoRA weights to prevent cross-level negative transfer.")
                        self.model.load_state_dict(_MODEL_CACHE[initial_state_key], strict=False)
                
                self.action_history.clear()  # Clear history for new level
                self.action_queue.clear()    # Clear queue for new level
                self.probed_coordinates.clear()  # Clear probed controls for new level
                self.control_colors = {1, 9}  # Reset persistent control colors for new level
                self.movable_color = None  # Reset persistent movable color for new level
                self.target_color = None  # Reset persistent target color for new level
                self.current_score = current_level

            # 3. Handle Game Reset
            if latest_frame.state in [GameState.NOT_PLAYED, GameState.GAME_OVER]:
                # 如果是 GAME_OVER 且有累積成功步軌跡，在 Reset 前微調學習該關卡解法
                if latest_frame.state == GameState.GAME_OVER and self.action_history:
                    ...
                    if positive_steps:
                        ...
                        if is_e2b:
                            print(f"[{self.game_id}] Level failed. Running online SFT on {len(positive_steps)} steps to learn level solution...")
                            try:
                                train_lora_online(self.model, self.processor, positive_steps, num_epochs=3, lr=1e-4)
                            except Exception as train_err:
                                print(f"[Online SFT Error] Failed to run SFT training on failure: {train_err}")
                
                self.action_history.clear()
                self.action_queue.clear()
                self.probed_coordinates.clear()
                self.control_colors = {1, 9}
            ...
            # 4. Execute next action from queue if queue is not empty
            if self.action_queue:
                action_tuple = self.action_queue.popleft()
                action_name, action_data, reasoning = action_tuple
                
                if action_name == "ACTION6":
                    action = GameAction.ACTION6
                    action.set_data(action_data)
                    self.probed_coordinates.add((int(action_data.get("x", 0)), int(action_data.get("y", 0))))
                else:
                ...
                if action.name == "ACTION6":
                    action_desc += f"(x={action_data.get('x', 0)}, y={action_data.get('y', 0)})"
                    
                step_record = {
                    "action_desc": action_desc,
                    "reasoning": reasoning,
                    "pdca_plan": "Queued BFS plan execution",
                    "pdca_check": "Executing multi-step sequence",
                    "pdca_act": "Continue sequence",
                    "prev_grid": np.array(latest_frame.frame[-1]),
                ...
                print(f"[{self.game_id}] Executing queued action: {action_desc} (Remaining queued: {len(self.action_queue)})")
                return action

            # 5. Build list of failed actions and count frequencies to prevent loops
            failed_actions = set()
            action_counts = {}
            for step in self.action_history:
                desc = step["action_desc"]
                action_counts[desc] = action_counts.get(desc, 0) + 1
                if step["outcome"] == "No change.":
                    failed_actions.add(desc)

            # Prevent action loops: if an action is repeated 3 or more times, treat it as failed/ruled out
            for desc, count in action_counts.items():
                if count >= 3:
                    failed_actions.add(desc)
                    base_name = desc.split("(")[0]
                    if base_name in ["ACTION1", "ACTION2", "ACTION3", "ACTION4", "ACTION5", "ACTION7"]:
                        failed_actions.add(base_name)

            # 4. Generate representations & Causal Analysis
            current_grid = np.array(latest_frame.frame[-1])
            grid_text = grid_to_text_representation(current_grid)

            available_actions = getattr(latest_frame, 'available_actions', None)
            avail_desc = "None listed"
            if available_actions:
                avail_desc = ", ".join(
                    f"ACTION{a}" if a != 6 else "ACTION6 (with x, y coords)"
                    for a in available_actions
                )

            # 4.5 Causal Transition Table Probing (First Principles System ID)
            ...
            # Determine control colors clicked historically
            if not self.control_colors:
                for step in self.action_history:
                    desc = step["action_desc"]
                    outcome = step.get("outcome")
                    if "ACTION6" in desc and outcome and "changed globally" in outcome:
                        try:
                            ...
                            cx = int(coords_str.split("x=")[1].split(",")[0])
                            cy = int(coords_str.split("y=")[1])
                            self.control_colors.add(int(step["prev_grid"][cy, cx]))
                        except Exception:
                            pass
            
            # Determine movable color and target color using robust adjacency-based logic
            ...
            # Identify target and movable colors based on adjacency of core colors (excluding 0, 5, etc.)
            ...
            # Segment Connected Components ignoring color (spatial connectivity only)
            visited = np.zeros((H, W), dtype=bool)
            objects = []
            dy = [-1, -1, -1,  0, 0,  1, 1, 1]
            dx = [-1,  0,  1, -1, 1, -1, 0, 1]
            ...
            for y_idx in range(H):
                for x_idx in range(W):
                    val = int(current_grid[y_idx, x_idx])
                    if val != bg_color and not visited[y_idx, x_idx]:
                        component = []
                        queue_bfs = [(y_idx, x_idx)]
                        visited[y_idx, x_idx] = True
                        head = 0
                        while head < len(queue_bfs):
                            cy, cx = queue_bfs[head]
                            head += 1
                            component.append((cy, cx))
                            for i in range(8):
                                ny, nx = cy + dy[i], cx + dx[i]
                                if 0 <= ny < H and 0 <= nx < W:
                                    if int(current_grid[ny, nx]) != bg_color and not visited[ny, nx]:
                                        visited[ny, nx] = True
                        ...
                        objects.append({
                            "colors": set(int(current_grid[py, px]) for py, px in component),
                            "pts": set(component),
                            "bbox": (min_y, max_y, min_x, max_x),
                            "w": w_size,
                            "h": h_size,
            ...
            def get_dominant_color(obj):
                colors_in_obj = [int(current_grid[py, px]) for py, px in obj["pts"] if int(current_grid[py, px]) != bg_color]
                if not colors_in_obj:
                    return None
                unique, counts = np.unique(colors_in_obj, return_counts=True)
                return unique[np.argmax(counts)]

            def simulate_click_local(grid_state, cx, cy):
                ...
                if shape_type in ["Single Cell", "Square Cluster / Dial Dial"] or (w_size <= 2 and h_size <= 2):
                    rotated_coords = []
                    for py, px in zip(m_ys, m_xs):
                        dy_diff, dx_diff = py - center_y, px - center_x
                        ny, nx = center_y + dx_diff, center_x - dy_diff
                        if 0 <= ny < H and 0 <= nx < W:
                            rotated_coords.append((ny, nx))
                    if rotated_coords:
                        new_grid[new_grid == movable_color] = bg_color
            ...
            # Dynamically detect wall color (typically 5, representing the static bounds)
            unique_colors, counts = np.unique(current_grid, return_counts=True)
            non_wall_colors = [bg_color, movable_color, target_color, 1, 9]
            wall_color = 5 # default fallback
            ...
            for idx in sorted_by_freq:
                ...
                if c_val not in non_wall_colors:
                    wall_color = c_val
                    break

            # Calculate Accurate Alignment Target Gap (separating teeth tips from goal markers)
            H, W = current_grid.shape
            ...
            if len(ys_t) > 0:
                    ...
                    for dy, dx in [(-1,0), (1,0), (0,-1), (0,1)]:
                        ...
                        if 0 <= ny < H and 0 <= nx < W:
                            if current_grid[ny, nx] == movable_color:
                                is_tooth = True
                            if current_grid[ny, nx] == wall_color:
                                is_target = True
                    if is_tooth:
                ...
                gap_text = (
                    f"- Translation Gap: dx={dx_gap:.1f} columns (needs shift left/right), dy={dy_gap:.1f} rows (needs shift up/down)\n"
                    f"- Goal orientation alignment: Compare movable shape color {movable_color} and target marker color {target_color}."
                )

            # Build Empirical Causal Transition Table based on REAL observation history (General System Identification)
            causal_lines = []
            seen_actions = set()
            ...
            for i in range(len(self.action_history)):
                ...
                # Retrieve prev_grid for this step
                grid_prev = step_data.get("prev_grid")
                
                # Retrieve post_grid (the grid after executing this action)
                if i + 1 < len(self.action_history):
                    grid_post = self.action_history[i+1].get("prev_grid")
                else:
                    grid_post = current_grid
                ...
                if grid_prev is not None and grid_post is not None:
                    try:
                        ...
                        if len(m_ys_prev) > 0 and len(m_ys_post) > 0:
                            ...
                            effect_desc = "No visible change (or shape temporarily vanished)"
                    except Exception as parse_err:
                        effect_desc = f"Error evaluating effect: {parse_err}"
                ...
                if step_data.get("outcome") == "No change.":
                    effect_desc += " [WARNING: Already failed with 'No change' previously!]"
            ...
            # Detect active buttons on current grid (colors 1 and 9)
            detected_buttons = []
            h, w = current_grid.shape
            scale_x = int(64 / w)
            scale_y = int(64 / h)
            ...
            for r in range(h):
                for c in range(w):
                    ...
                    if val in [1, 9] and not visited_btn[r, c]:
                        comp = []
                        q = [(r, c)]
                        ...
                        detected_buttons.append({
                            "color": val,
                            "grid_center": (center_gx, center_gy),
            ...
            if detected_buttons:
                ...
                buttons_text += "The game grid contains active controls (buttons/sliders). Clicking outside these controls has no effect.\n"
            ...
            system_content = (
                ...
                "   - Compare the main movable shape and the target marker: Are their orientations/angles different? (If yes, you need to find a rotation mechanism). Are their sizes/lengths different? (If yes, you need to find a scaling/stretch mechanism).\n"
                ...
                "   - Function: This is a slider interface. Clicking different positions along it (e.g. left side vs right side) usually extends, shrinks, or translates the controlled object.\n"
                "   - Operation Sensitivity: You must click coordinate points inside the slider line itself to slide it. Click different positions along the slider line to test different directions of adjustment.\n"
                ...
                "   - Active controls (buttons, dials, sliders) are spatially separated from passive goal markers. Clicking passive goal markers or target points usually does nothing. Focus your actions strictly on coordinates within the active button, dial, or slider structures.\n"
                "5. Local vs Global Action-Reaction Duality:\n"
                "   - A click may cause a local change (the button itself toggles color) or a global geometric change (a shape somewhere else moves or rotates).\n"
                "   - Your goal is to identify which buttons cause global geometric transformations on the main movable shape to align it with the target markers.\n\n"
                "=== PDCA LOOP INSTRUCTION ===\n"
                "For every step, you must explicitly construct and update your PDCA loop:\n"
                "  - Plan (P): Analyze shape alignment gaps (orientation difference, size difference). Identify suspected slider or rotary dial layouts on the grid. Plan an action to test if a specific control dial/slider changes the orientation or length of the main shape.\n"
                "  - Do (D): Output the chosen action (discrete or coordinate click).\n"
                "  - Check (C): Did the action cause a local button color toggle, or a global shape change (rotation, translation, stretch)? Does the current state decrease the alignment gap to the target?\n"
                "  - Act (A): Adjust strategy. Focus on controls that trigger global shape transformations, and avoid wasting turns toggling local buttons that don't affect the main shape.\n\n"
                ...
                "}\n"
                "In candidate_control_coords, list up to 5 coordinates (x, y) of controls (sliders, rotary dials, buttons) you want the agent to simulate and optimize using a local multi-step sequence search.\n"
                "RULES: pdca_plan, pdca_check, and pdca_act MUST be in Traditional Chinese (繁體中文).\n"
                "NEVER repeat an action that already resulted in 'No change'."
            )



            # 6.5. Information Entropy Router (Gemma-4-E2B-it for Scanning, Gemma-4-12b-it for Execution)
            control_objects = []
            for obj in objects:
                dom_color = get_dominant_color(obj)
                is_control = (dom_color is not None and dom_color != target_color and dom_color != movable_color)
            ...
            for obj in control_objects:
                ...
                for cx, cy in self.probed_coordinates:
                    if min_y - 1 <= cy <= max_y + 1 and min_x - 1 <= cx <= max_x + 1:
                        has_clicked = True
                        break
                if has_clicked:
                    probed_count += 1
                    
            total_controls = len(control_objects)
            if total_controls == 0:
                entropy = 0.0
            else:
                entropy = 1.0 - (probed_count / total_controls)
            ...
            if entropy > 0.2:
                ...
                phase_desc = (
                    ...
                    f"- Goal: Probe unclicked control objects to identify their causal effects and complete the transition table."
                )
            else:
                ...
                phase_desc = (
                    ...
                    f"- Goal: The causal table is complete. Execute the optimal sequence of clicks to align the movable shape with the targets."
                )
                
            active_model, active_processor = get_gemma4_model_and_processor(model_id, self.device)
            
            # Reset LoRA weights if game changed
            prev_game_id = _MODEL_CACHE.get("current_game_id")
            if prev_game_id != self.game_id:
                initial_state_key = f"initial_lora_state_{model_id}"
                if initial_state_key in _MODEL_CACHE:
                    print(f"[LoRA Reset] Game changed from {prev_game_id} to {self.game_id}. Resetting LoRA weights for {model_id} to initial state.")
                    active_model.load_state_dict(_MODEL_CACHE[initial_state_key], strict=False)
            
            # Inject phase description into user content
            user_content_text = (
                f"{phase_title}\n"
                f"{phase_desc}\n\n"
                f"{user_content_text}"
            ...
            if failed_actions:
                failed_desc = "\n".join(f"- {act}" for act in sorted(failed_actions))
                user_content_text += (
                    f"\n\nCRITICAL: The following actions resulted in 'No change' previously in this level. "
                    f"DO NOT repeat them! You MUST choose a different command:\n{failed_desc}"
                )
            ...
            # 7. Apply prompt template and preprocess inputs
            ...
            # (已移除舊的 BFS 劫持與硬編碼模擬對齊機制，回歸一般化的大模型規劃與決策)

            # Map action back to description for history tracking
            action_desc = action.name
            if action.name == "ACTION6" and hasattr(action, "action_data") and action.action_data:
                data = action.action_data.model_dump()
                action_desc += f"(x={data.get('x', 0)}, y={data.get('y', 0)})"

            # 11. Overriding Heuristic: Intercept and swap failed actions to ensure exploration
            if action_desc in failed_actions or action.name in failed_actions:
                print(f"[{self.game_id}] Heuristic Intercept: Model choice '{action_desc}' has already failed. Intercepting...")
                
                # First, record the model's actual failed choice in history as "No change."
                # This feeds the correct failure feedback back to the model's context in the next step.
                step_record = {
                    "action_desc": action_desc,
                ...
                override_action = self.choose_rareness_exploration_action(current_grid, failed_actions, available_actions)
                if override_action is not None:
                    override_desc = override_action.name
                    if override_action.name == "ACTION6" and hasattr(override_action, "action_data") and override_action.action_data:
                        ov_data = override_action.action_data.model_dump()
                        self.probed_coordinates.add((int(ov_data.get("x", 0)), int(ov_data.get("y", 0))))
                        override_desc += f"(x={ov_data.get('x', 0)}, y={ov_data.get('y', 0)})"
                    
                    override_step = {
                        "action_desc": override_desc,
                        "reasoning": getattr(override_action, "reasoning", "Fallback exploration"),
                        "pdca_plan": "Fallback exploration due to model repeat/fail",
                        "pdca_check": "Checking fallback outcome",
                        "pdca_act": "Continue exploration",
                        "prev_grid": current_grid,
                        "prev_score": current_level,
                        "outcome": None,
                        "prompt": "",
                    ...
                    self.action_history.append(override_step)
                    if len(self.action_history) > 15:
                        self.action_history.pop(0)
                    
                    print(f"[{self.game_id}] Heuristic Intercept: Executing fallback exploration: '{override_desc}' (Reasoning: {override_action.reasoning})")
                    return override_action
                else:
                    override_action = GameAction.RESET
                    override_action.reasoning = "Reset game due to exploration exhaustion."
                    print(f"[{self.game_id}] Heuristic Intercept: Executing fallback exploration: RESET")
                    return override_action

            # 12. Record step in history
            step_record = {
                "action_desc": action_desc,
                "reasoning": getattr(action, "reasoning", "No reasoning provided"),
                "pdca_plan": getattr(action, "pdca_plan", ""),
            ...
            self.action_history.append(step_record)
            if len(self.action_history) > 15:
                self.action_history.pop(0)  # Keep window size 15
