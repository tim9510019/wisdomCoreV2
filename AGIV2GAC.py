...
class AGIV2LocalBlock(nn.Module):
    def __init__(
        self, D=1152, hidden_dim=6912, C=1024, num_heads=4, num_kv_heads=1, head_dim=256
    ):
        super().__init__()
        self.C = C
    ...
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        causal_mask = torch.tril(
            torch.ones((self.C, self.C), device=device, dtype=torch.bool)
        )
        ...
        attn_weights = attn_weights.masked_fill(
            ~sdpa_mask, torch.finfo(attn_weights.dtype).min
        )

        attn_probs = F.softmax(attn_weights, dim=-1)

        Z_hat_chunked = torch.matmul(attn_probs, V_c)
        Z_hat_chunked = Z_hat_chunked.transpose(2, 3).reshape(
            B, L_pad, self.num_heads * self.head_dim
        )

        Z_hat_shifted = self.o_proj_loc(Z_hat_chunked)
        Z_hat = (
            torch.roll(Z_hat_shifted, shifts=shift_size, dims=1)
            if shift_size > 0
            else Z_hat_shifted
        )
...
class AGIV2GlobalBlock(nn.Module):
    def __init__(
        ...
        # 🚀 Gemma 3 的四重 LayerNorm 架構 (負責主要資訊流)
        self.input_layernorm = GemmaRMSNorm(D)
        self.post_attention_layernorm = GemmaRMSNorm(D)
        self.pre_feedforward_layernorm = GemmaRMSNorm(D)
        self.post_feedforward_layernorm = GemmaRMSNorm(D)

        # 🚀 額外擴充：為自定義的 FFT 與 Mem 分支配備獨立 Norm
    ...
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        Q_exp = self.Q_mem.view(1, 1, self.M, D).expand(
            B, N_mem, -1, -1
        )  # [B, N, M, D]
        ...
        if past_mask is not None:
            ...
            safe_past_mask = (
                past_mask_chunks | ~chunk_valid
            )  # 避免全 False 導致 Softmax NaN
        ...
        # [Phase III-b] Per-Chunk Cross Attention (無因果限制：共享全局 M_global)
        Z_hat_chunks = Z_hat.view(B, N, self.C, -1)  # [B, N, C, D']

        Q_cross = self.W_q_cross(Z_hat_chunks)  # [B, N, C, nh*hd]
        Q_cross = Q_cross.view(B * N, self.C, self.num_heads, self.head_dim)

        # 🚀 替換為無因果限制的 M_global
        K_cross = self.W_k_cross(M_global)  # [B, N, M, nkv*hd]
        K_cross = K_cross.view(B * N, self.M, self.num_kv_heads, self.head_dim)

        V_cross = self.W_v_cross(M_global)  # [B, N, M, nkv*hd]
