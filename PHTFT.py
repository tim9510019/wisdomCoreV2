...
def apply_rotary_emb(xq, xk, freqs_cos, freqs_sin):
    ...
    xq_out = torch.stack([xq_r * f_cos - xq_i * f_sin, xq_r * f_sin + xq_i * f_cos], dim=-1).flatten(3)
    xk_out = torch.stack([xk_r * f_cos - xk_i * f_sin, xk_r * f_sin + xk_i * f_cos], dim=-1).flatten(3)
...
class AlignedHologramLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        V = self.v_linear(x)
        Q_rot, K_rot = apply_rotary_emb(sqk.unsqueeze(2), (sqk * self.k_scale).unsqueeze(2), self.freqs_cos[:L], self.freqs_sin[:L])
        
        Q_f, K_f, V_f = torch.fft.rfft(Q_rot.squeeze(2), dim=1), torch.fft.rfft(K_rot.squeeze(2), dim=1), torch.fft.rfft(V, dim=1)
...
class AlignedAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        Q_rot, K_rot = apply_rotary_emb(sqk.view(B, L, self.num_heads, self.head_dim), 
                                        (sqk * self.k_scale).view(B, L, self.num_heads, self.head_dim), 
                                        self.freqs_cos[:L], self.freqs_sin[:L])
        scores = torch.matmul(Q_rot.transpose(1, 2), K_rot.transpose(1, 2).transpose(-2, -1)) / (self.head_dim ** 0.5)
...
class HoloTferHybrid(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_heads, hidden_dim, max_seq_len=512):
        ...
        self.h_ffn = nn.Sequential(nn.Linear(embed_dim, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, embed_dim))
        ...
        self.t_ffn = nn.Sequential(nn.Linear(embed_dim, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, embed_dim))
...
def main():
    ...
    print(f"Hybrid HoloTfer 總參數量: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"Epoch [{epoch+1}/{NUM_EPOCHS}], Loss: {total_loss/len(loader):.4f}, Acc: {correct/5000:.4f}")
