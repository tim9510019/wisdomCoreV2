...
# =================================================================
from AGI import HolographicMemoryNetwork as AGINetwork
from AGIV2 import HolographicMemoryNetwork as AGIV2Network
...
def create_training_plot(results_map, seq_len, filename):
    colors_map = {'AGI': 'r', 'AGIV2': 'b'} # 已移除 PHTF 的顏色對應
...
def build_presentation_all_in_one(summary_data, plot_files, param_data, base_dir):
    ...
    slide.placeholders[1].text = "AGI vs AGIV2\nSequence Lengths Scanning (32 to 8192)"
    ...
    rows = len(SEQ_LENS) + 1
    # 調整表格為 3 欄
    table = slide.shapes.add_table(rows, 3, Inches(1.0), Inches(1.5), Inches(8.0), Inches(4.0)).table
    for i, h in enumerate(['Seq Length', 'AGI', 'AGIV2']): table.cell(0, i).text = h
    for r_idx, seq in enumerate(SEQ_LENS):
        table.cell(r_idx + 1, 0).text = str(seq)
        for c_idx, model_name in enumerate(['AGI', 'AGIV2']):
    ...
    out_file = os.path.join(base_dir, "Task21_All_In_One_Report_AGI_vs_AGIV2.pptx")
    prs.save(out_file)
    return out_file

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"啟動 Task 21 NIAH 掃描式測試 (AGI vs AGIV2)... {device}")
    
    base_dir = os.path.dirname(os.path.abspath(__file__))
    summary_data = {'AGI': {}, 'AGIV2': {}}
    param_data = {'AGI': {}, 'AGIV2': {}}
    plot_files = {}
    ...
    for seq in SEQ_LENS:
        ...
        plot_path = os.path.join(base_dir, f'task21_scan_seq{seq}_agi_vs_AGIV2.png')
        create_training_plot(seq_results, seq, plot_path)
        plot_files[seq] = plot_path

    print("\n產生 All-In-One 簡報檔...")
    pptx_path = build_presentation_all_in_one(summary_data, plot_files, param_data, base_dir)
