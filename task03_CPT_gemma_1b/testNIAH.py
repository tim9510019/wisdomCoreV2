...
def generate_haystack_with_needle(
    tokenizer, target_length, depth_ratio, needle, question
):
    ...
    required_filler_len = max(
        0, target_length - len(needle_tokens) - len(question_tokens) - 5
    )
...
def generate_custom(model, input_ids, max_new_tokens=20, eos_token_id=None):
    ...
    for _ in range(max_new_tokens):
        ...
        logits = (
            outputs.get("logits", outputs) if isinstance(outputs, dict) else outputs
        )
...
def run_niah_test(mode="base", checkpoint_path=""):
    ...
    question = (
        "What is the secret activation code for the project? Answer only with the code."
    )
    ...
    for i, length in enumerate(lengths):
        for j, depth in enumerate(depths):
            print(
                f"[{i+1}/{len(lengths)}] 長度: {length}, 深度: {depth*100:.0f}% ... ",
                end="",
            )

            prompt = generate_haystack_with_needle(
                tokenizer, length, depth, needle, question
            )
            input_ids = tokenizer(
            ...
            is_correct = (
                1.0 if expected_answer.lower() in generated_ans.lower() else 0.0
            )
            results[i, j] = is_correct

            status = (
                "✅ 成功" if is_correct else f"❌ 失敗 (回答: {generated_ans.strip()})"
            )
            print(status)
...
if __name__ == "__main__":
    ...
    lengths, depths, results = run_niah_test(
        mode=args.mode, checkpoint_path=args.checkpoint
    )
