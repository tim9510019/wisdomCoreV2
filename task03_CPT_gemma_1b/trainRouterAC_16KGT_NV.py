# trainRouterAC_16KGT_NV.py — 物理隔離對齊終極版 (N -> N+B 範式 + GT 動態 Router + NV 雙卡配置)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agiv2_zerogate_ac_checkpoints_16KGT_NV"
LOG_PATH = "~/zerogate_32k_ac_16kgt_nv_log.csv"
BEST_8K_PATH = "~/agiv2_zerogate_ac_checkpoints_8KGT_NV/best_model.pth"

SAVE_DIR = os.path.expanduser(SAVE_DIR)
LOG_PATH = os.path.expanduser(LOG_PATH)
BEST_8K_PATH = os.path.expanduser(BEST_8K_PATH)
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1B-Router_16KGT_NV"
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 Router 16K 訓練矩陣 (原生 Gemma 3 權重起步 + 16K GT)...")
    ...
    # 🌟 載入 8K 階段的 best_model 權重
    if os.path.exists(BEST_8K_PATH):
        print(
            f"\n🔄 成功尋獲 8K 階段之最佳權重，正在載入以進行 16K 延伸訓練: {BEST_8K_PATH}"
        )
        state_dict = torch.load(BEST_8K_PATH, map_location="cpu")
        model.load_state_dict(state_dict, strict=False)
    else:
        print(f"\n⚠️ 找不到 8K 權重檔案 {BEST_8K_PATH}，將從頭開始訓練！")
    ...
    final_path = os.path.join(SAVE_DIR, "final_router_ac_16kgt.pth")
    torch.save(
        (
            model.state_dict()
