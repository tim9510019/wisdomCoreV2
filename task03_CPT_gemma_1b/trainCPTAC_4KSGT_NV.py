# trainCPTAC_4KSGT_NV.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版 + 雙卡 4K)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agiv2_cpt_checkpoints_4KS_NV"
LOG_PATH = "~/agiv2_cpt_4KS_NV_log.csv"
BEST_1KS_NV_PATH = "~/agiv2_cpt_checkpoints_1KS_NV/best_cpt_model.pth"

DATASET_DIR = os.path.expanduser(DATASET_DIR)
SAVE_DIR = os.path.expanduser(SAVE_DIR)
LOG_PATH = os.path.expanduser(LOG_PATH)
BEST_1KS_NV_PATH = os.path.expanduser(BEST_1KS_NV_PATH)
...
# 🌟 4K 環境記憶體優化：單卡 Batch Size 設為 1，累加步數設為 16 (雙卡環境下 global batch size 等同 32)
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1B-CPT_4KSGT_NV"
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 第二階訓練矩陣 (動態 MoE 路由版 雙卡 4K)...")
    ...
    if os.path.exists(BEST_1KS_NV_PATH):
        print(f"\n🔄 載入 1KS NV 階段基礎權重: {BEST_1KS_NV_PATH}")
        model.load_state_dict(torch.load(BEST_1KS_NV_PATH, map_location="cpu"), strict=False)
        print("✅ 路由權重載入完成。")
    else:
        print(f"\n⚠️ 找不到 1KS NV 路由權重檔案 {BEST_1KS_NV_PATH}，將完全從零開始訓練！")
    ...
    if last_checkpoint is not None:
        print(f"\n🚀 偵測到 4K SGT 時間錨點 {last_checkpoint}，啟動狀態無損恢復...")
        trainer.train(resume_from_checkpoint=last_checkpoint)
    else:
        print("\n🚀 開始全新 4K SGT 訓練...")
        trainer.train()
        
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_4ksgt.pth"))
