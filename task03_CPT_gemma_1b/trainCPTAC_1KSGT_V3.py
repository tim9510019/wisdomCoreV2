...
SAVE_DIR = "./agiv3_cpt_checkpoints_1KSGT"
LOG_PATH = "./agiv3_cpt_1kSGT_log.csv"
...
# [ 動態 Byte 轉換與絕對斷層拼接器 ]
# ==========================================
class PureByteDataCollator:
    def __init__(self, pad_byte=0):
    ...
    def __call__(self, features):
        ...
        for inputs, labels in zip(batch_input_ids, batch_labels):
            pad_len = max_len - len(inputs)
            padded_inputs.append(inputs + [self.pad_byte] * pad_len)
...
class ThermalControlCallback(TrainerCallback):
    def __init__(self, delay_seconds=2.3):
...
class HFAutoUploadCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        # eval_loss 是純 CE loss（eval 時不加 entropy 懲罰）
        ce_loss = metrics.get("eval_loss", None)
        if ce_loss is None:
            return

        # 觸發條件：eval_loss 創新低 且 低於門檻值
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV3 (Byte Latent) CPT 訓練矩陣 (動態 MoE 路由版)...")

    parquet_file = os.path.join(DATASET_DIR, "agiv3_raw_text.parquet")
    ...
    print("✅ AGIV3 漏斗型實例化完成，字典依賴已徹底抹除。")
