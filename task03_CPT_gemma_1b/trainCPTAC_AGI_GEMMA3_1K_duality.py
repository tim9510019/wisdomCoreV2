# trainCPTAC_AGI_GEMMA3_1K_duality.py — AGI GEMMA 3 1D Depthwise Conv + Sinc Causal 波粒二象性自適應規範場預訓練引擎 (GEMM 全凍結，745,472 可訓練參數，物理隔離)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_cpt_checkpoints_1K_duality"
LOG_PATH = "~/agigemma3_cpt_1k_duality_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-CPT_1K_duality"
HF_CE_LOSS_THRESHOLD = 3.9  # 只有 ce_loss 低於此值才觸發上傳


# ==========================================
# [ 物理隔離：時域 Sinc Causal 卷積模組 ]
# ==========================================
class SincCausalRoPE(nn.Module):
    def __init__(
        self, num_layers=26, num_heads=4, head_dim=256, target_params=745472, W=64
    ):
        """
        物理隔離的獨立調製模組。
        內部封裝一個長度完全精準相等的 745,472 可訓練參數張量，
        為每層分配 28,672 個參數（變形為 112 x 256 的相位調製矩陣），
        透過 1D Causal Sinc 卷積 (Depthwise Conv1d) 在時域上提取特徵並生成動態相位。
        """
        super().__init__()
    ...
    def forward(self, x, layer_idx):
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            fc = torch.linspace(
                0.02, 0.48, steps=256, device=device, dtype=torch.float32
            )
            ...
            kernel = 2.0 * fc.unsqueeze(1) * sinc_val * window
            ...
            h_modulated = F.conv1d(h_padded, weight_conv, groups=256)
            h_modulated = h_modulated.transpose(1, 2)  # [B, L, 256]

            h_expanded = torch.cat([h_modulated, h_modulated], dim=-1)
...
# =======================================================
# [ 🌊 波粒二象性調製器（Wave-Particle Duality Operator）]
# =======================================================
def apply_dual_wave_particle_rope(x, head_dim, base_freq=1000000.0, delta_theta=None):
    ...
    theta_base = freqs.unsqueeze(0).unsqueeze(2)  # shape: (1, L, 1, 128)

    if delta_theta is not None:
        # 2. 物理勘測（無參數開銷）：利用 delta_theta 通道維度的標準差度量結構相干性
        # delta_theta shape: (B, L, num_heads, 128)
        spectral_std = torch.std(
            delta_theta, dim=-1, keepdim=True
        )  # shape: (B, L, num_heads, 1)

        # 3. 構造自適應規範場密度規範閘 kappa
        # 信號定域諧振強（標準差大）時 kappa -> 0，塌縮為定域粒子態，完美釋放絕對座標外推力
        # 信號充滿擾動雜訊（標準差小）時 kappa -> 1，絕對座標解鎖，進入純波動諧振流形強制低通抗噪
        kappa = 1.0 - torch.tanh(spectral_std)

        # 4. 正反合代數一體化流形合流
        emb = (1.0 - kappa) * theta_base + delta_theta
    else:
        emb = theta_base
...
# [ 動態修補前向傳播：接合波粒二象性規範場 ]
# ==========================================
def make_dual_fusion_decoder_forward(layer_idx, fft_phase_lock_module):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        # 調用 100% 同等參數量物理隔離模組獲取波動分量
        delta_theta = fft_phase_lock_module(normed_X, layer_idx)

        # 進行波粒二象性自適應融合調製
        Q = apply_dual_wave_particle_rope(
            Q, self.head_dim, base_freq=self.rope_base, delta_theta=delta_theta
        ).contiguous()

        delta_theta_k = delta_theta.mean(dim=2, keepdim=True)
        K = apply_dual_wave_particle_rope(
            K, self.head_dim, base_freq=self.rope_base, delta_theta=delta_theta_k
        ).contiguous()
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 CPT 訓練矩陣 (波粒二象性 Duality 1K 版本，GEMM 全凍結)...")
    ...
    # 實例化物理隔離的 Sinc Causal 調製模組 (維持 745,472 封閉基準)
    print(
        f"\n🔮 正在初始化物理隔離之 Sinc Causal 調製模組 (總參數量精準等於 745,472)..."
    )
    fft_phase_lock = SincCausalRoPE(
        num_layers=26, num_heads=4, head_dim=256, target_params=745472
    )

    # 動態修補 (Monkey Patch) 所有 Block 的 forward 以引入波粒相干自適應規範場
    print("🛠️ 正在動態修補模型解碼塊 (Gemma3DecoderLayer)，接合規範場 Duality 邏輯...")
    for idx, block in enumerate(base.blocks):
        block.forward = types.MethodType(
            make_dual_fusion_decoder_forward(idx, fft_phase_lock), block
        )
    ...
    print("✅ 模型實例化完成，無參數開銷波粒統一調製器已順利掛載。")

    # 🔒 鎖定全局預訓練參數，僅開放實體隔離之參數更新
    print("\n🔒 鎖定全局預訓練參數，僅開放波粒自適應模組參數更新...")
    for param in model.parameters():
        param.requires_grad = False

    for name, param in model.named_parameters():
        if "fft_phase_lock" in name:
    ...
    print(f"🟢 可訓練參數總量 (Duality): {trainable_params:,} (預期應精準等於 745,472)")

    if trainable_params != 745472:
        print(
            f"⚠️ [警告] 可訓練參網路量 ({trainable_params:,}) 不等於預期的對照基準 (745,472)，請重新檢查架構！"
        )
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_cpt_1k_duality.pth"),
    )
