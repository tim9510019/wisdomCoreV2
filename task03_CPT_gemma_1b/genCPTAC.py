...
# --- 1. 路徑與基礎設定 ---
MODEL_ID = (
    "google/gemma-3-1b-it"  # 若已重新使用 pt 版訓練，可直接改為 "google/gemma-3-1b-pt"
)
...
def load_cpt_model():
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        print(
            f"⚠️ 找不到權重檔：{CHECKPOINT_PATH}，請確認防作弊版本的訓練是否已產出檔案。"
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if len(prompt_ids) > MAX_PROMPT_LENGTH:
        print(
            f"✂️ [注意] 原始上下文長度 ({len(prompt_ids)}) 超出負載限制，自動截斷保留最靠近生成的後 {MAX_PROMPT_LENGTH} 個 Token。"
        )
