# trainCPTAC_4KSGT.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版 + 4K視界 + 主動式散熱)
import os
import sys
import csv
import time
...
SAVE_DIR = "./agiv2_cpt_checkpoints_4KSGT"
LOG_PATH = "./agiv2_cpt_4kSGT_log.csv"
BEST_1KSGT_PATH = "./agiv2_cpt_checkpoints_1KSGT/best_cpt_model.pth"
...
# 4K 記憶體消耗極大，Batch Size 下調，Grad Accumulation 等比例放大
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1300M-blackwell-CPT_GT_4K"
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 第二階訓練矩陣 (動態 MoE 路由版 4K長度)...")
    ...
    if os.path.exists(BEST_1KSGT_PATH):
        print(f"\n🔄 載入 1K 階段基礎權重: {BEST_1KSGT_PATH}")
        model.load_state_dict(torch.load(BEST_1KSGT_PATH, map_location="cpu"), strict=False)
        print("✅ 1K 權重載入完成。")
    else:
        print(f"\n⚠️ 找不到 1K 權重檔案 {BEST_1KSGT_PATH}，將完全從零開始訓練！")
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ...
            ThermalControlCallback(delay_seconds=2.3), # 🛡️ 植入硬體保護控制器
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_4k_sgt.pth"))
