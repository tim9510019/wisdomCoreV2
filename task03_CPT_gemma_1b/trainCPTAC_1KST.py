# trainCPTAC_1K.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版)
import os
import sys
import csv
import time
...
class QuantumCPTMonitor(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if logs:
            ...
            # # 終端機直觀顯示三國演義的勢力變化 (總和永遠趨近於 1.0)
            # if state.global_step % LOGGING_STEPS == 0:
            #     print(f"📊 [Gate 動態 | Step {state.global_step}] Loc: {g_loc_avg:.4f} | Mem: {g_mem_avg:.4f} | FFT: {g_fft_avg:.4f}")
