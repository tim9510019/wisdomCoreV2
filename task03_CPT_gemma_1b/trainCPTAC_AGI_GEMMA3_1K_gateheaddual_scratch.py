# trainCPTAC_AGI_GEMMA3_1K_gateheaddual_scratch.py — AGI GEMMA 3 動態閘門頭級子空間解耦 gateHeadDual From-Scratch 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_gateheaddual"
LOG_PATH = "~/agigemma3_scratch_1k_gateheaddual_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_gateheaddual"
...
# ==========================================
# [ gateHeadDual 50/50 Sinc Phase Modulation & Subspace Gate Module ]
# ==========================================
class GatedSincCausalRoPE(nn.Module):
    def __init__(self, num_layers=26, target_params=745472, W=64):
        """
        gateHeadDual 專用的聯合調製與門控模組。
        額外參數量精確限制在 745,472。在每一層中進行正交切分：
        1. 波動頭 Sinc 卷積投影權重: 152 x 128 = 19,456 參數
        2. 動態子空間閘門投影權重: 1152 x 8 = 9,216 參數 (無 bias=False，精確對齊)
    ...
    def forward(self, x, layer_idx):
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            # 對 8 維特徵取均值並經過 Sigmoid，獲得自適應標量增益 gamma
            gamma = torch.sigmoid(torch.mean(s, dim=-1, keepdim=True)) # shape: (B, L, 1)
            
        return delta_theta.to(dtype), gamma.to(dtype)
...
# ==========================================
# [ 動態修補前向傳播：gateHeadDual 前向 ]
# ==========================================
def make_gated_head_decoupled_decoder_forward(layer_idx, gated_module):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        # 4. 波動頭：套用 Pure FRA 與動態自適應閘門計算
        delta_theta, gamma = gated_module(normed_X, layer_idx) # delta_theta: (B, L, 1, 128), gamma: (B, L, 1)
        ...
        # 6. 動態增益阻尼 (Dynamic Damping)：將 gamma 廣播乘入 Wave Head 注意力輸出
        attn_part = attn_out[:, :, :3] # shape: (B, L, 3, 256)
        attn_wave = attn_out[:, :, 3:] # shape: (B, L, 1, 256)
        
        attn_wave_gated = attn_wave * gamma.unsqueeze(-1) # shape: (B, L, 1, 256)
        
        # 7. 重組後進行投影
        attn_out_gated = torch.cat([attn_part, attn_wave_gated], dim=2)
...
def main():
    print(f"🔮 初始化 GEMMA 3 1B gateHeadDual (動態閘門頭級解耦) From-Scratch 訓練引擎...")
    ...
    # 實例化 gateHeadDual 調製與門控模組 (params: 745,472)
    gated_module = GatedSincCausalRoPE(num_layers=26, target_params=745472)
    
    for idx, block in enumerate(base.blocks):
        block.forward = types.MethodType(make_gated_head_decoupled_decoder_forward(idx, gated_module), block)
    ...
    print(f"🟢 可訓練參數總量 (From Scratch gateHeadDual): {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_gateheaddual.pth"),
    )
