# trainCPTAC_AGI_GEMMA3_1K_PUREX.py — AGI GEMMA 3X (頻域下採樣降維 + PURE 模式) 實驗
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_cpt_checkpoints_1K_PUREX"
LOG_PATH = "~/agigemma3_cpt_1k_PUREX_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-CPT_1K_PUREX"
...
def main():
    print(
        f"\n🚀 啟動 AGIGEMMA3X CPT 訓練矩陣 [PUREX 模式] (前 {N_FFT_F} 後 {N_FFT_B} 層 PureFFT + Gemma3 {DOWNSAMPLE_FACTOR}倍降維與權重凍結)..."
    )
    ...
    base = AGIGEMMA3X(
        ...
        fft_block_type="pure",              # 🌟 PUREX 模式使用 pure
