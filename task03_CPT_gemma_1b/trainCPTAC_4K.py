        ...
        if logs:
            ...
            if current_loss and current_loss < self.best_loss:
                ...
                if model is not None and state.global_step % (SAVE_STEPS // 2) == 0:
                    best_model_path = os.path.join(self.save_dir, "best_cpt_model_4k.pth")
                    raw_model = model.module if hasattr(model, 'module') else model
...
def main():
    ...
    if not os.path.exists(parquet_file):
        raise FileNotFoundError(f"找不到 4K 實體矩陣 {parquet_file}，請先執行 cpt4k.py。")
    ...
    print(f"✅ 資料隔離完成！訓練集: {len(train_ds):,} 筆 | 獨立驗證集: {len(eval_ds)} 筆")
