# trainCPTAC_AGI_GEMMA3_1K_loraduality_scratch.py — AGI GEMMA 3 LoRA + 波粒二象性自適應規範場 Hybrid From-Scrack 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_loraduality"
LOG_PATH = "~/agigemma3_scratch_1k_loraduality_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_loraduality"
...
class SincCausalRoPE(nn.Module):
    ...
    def forward(self, x, layer_idx):
        ...
        # 強制以 float32 進行穩定投影與 Sinc 卷積核構造
...
def apply_dual_wave_particle_rope(x, head_dim, base_freq=1000000.0, delta_theta=None):
    ...
    if delta_theta is not None:
        # 2. 物理勘測：利用 delta_theta 通道維度的標準差度量結構相干性
        spectral_std = torch.std(delta_theta, dim=-1, keepdim=True) # shape: (B, L, num_heads, 1)
...
def make_dual_fusion_decoder_forward(layer_idx, fft_phase_lock_module):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        # 調用 50% 參數量物理隔離模組獲取波動分量
        delta_theta = fft_phase_lock_module(normed_X, layer_idx)
        
        # 進行波粒二象性自適應融合調製
        Q = apply_dual_wave_particle_rope(Q, self.head_dim, base_freq=self.rope_base, delta_theta=delta_theta).contiguous()
        
        delta_theta_k = delta_theta.mean(dim=2, keepdim=True)
        K = apply_dual_wave_particle_rope(K, self.head_dim, base_freq=self.rope_base, delta_theta=delta_theta_k).contiguous()
...
class HFAutoUploadCallback(TrainerCallback):
    ...
    def _do_upload(self, ce_loss_val, step):
        with self._upload_lock:
            try:
                ...
                # 限制上傳頻率，每 1000 步或者極早期 (<= 100) 才上傳巨型模型檔案
                should_upload_model = (step % 1000 == 0) or (step <= 100)
                ...
                if should_upload_model:
                    ...
                    print(f"\n[HF Upload] ℹ️ 步數為 {step} (非 1000 的倍數)，跳過 2.5GB 模型檔案上傳以節省頻寬。")
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if ce_loss < self.best_ce_loss and ce_loss < self.ce_loss_threshold:
            ...
            t = threading.Thread(target=self._do_upload, args=(ce_loss, step), daemon=True)
            t.start()
...
# ==========================================
def main():
    print(f"🔮 初始化 GEMMA 3 1B LoRA + Wave-Particle Duality Hybrid 訓練引擎...")
    ...
    if not os.path.exists(parquet_file):
        raise FileNotFoundError(f"找不到實體庫 {parquet_file}")
    ...
    # 2. 注入 50% 參數量的時域 Causal Sinc 卷積二象性調製器 (params=372,736)
    ...
    # 3. 動態 Patch 解碼層的前向傳播方法
    print("🛠️ 正在動態修補模型解碼塊 (Gemma3DecoderLayer)，接合波粒二象自適應融合...")
    for idx, block in enumerate(base.blocks):
        block.forward = types.MethodType(make_dual_fusion_decoder_forward(idx, fft), block)
        
    # 4. 封裝 causal LM 包裝器
    model = AGIV2GForCausalLMT(base, use_gc=True, gate_entropy_lambda=GATE_ENTROPY_LAMBDA)
    model.fft_phase_lock = fft
    print("✅ 模型實例化完成，LoRA 與波粒自適應規範場已順利掛載。")
    
    # 5. FROM SCRATCH: 開放全局所有參數進行從頭訓練
    ...
    if last_checkpoint is not None:
        print(f"\n🚀 偵測到歷史 Checkpoint: {last_checkpoint}，啟動熱重啟復原...")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_loraduality.pth"),
    )
