# trainCPTAC_AGI_GEMMA3_1K_lorafra_scratch.py — AGI GEMMA 3 LoRA + FRA 去RoPE化雙效混合 From-Scratch 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_lorafra"
LOG_PATH = "~/agigemma3_scratch_1k_lorafra_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_lorafra"
...
# ==========================================
# [ 物理隔離：時域 Sinc Causal 卷積 FRA 模組 (50% 參數: 372,736) ]
# ==========================================
class SincCausalRoPE(nn.Module):
    def __init__(
        self, num_layers=26, num_heads=4, head_dim=256, target_params=372736, W=64
    ):
        """
        物理隔離的獨立調製模組。
        內部封裝一個長度完全精準相等的 372,736 可訓練參數張量，
        為每層分配 14,336 個參數（變形為 56 x 256 的相位調製矩陣），
        透過 1D Causal Sinc 卷積 (Depthwise Conv1d) 在時域上提取特徵並調製相位角度。
        """
        super().__init__()
    ...
    def forward(self, x, layer_idx):
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            h = torch.matmul(x_slice, w_layer_cast)
            ...
            h_modulated = h_modulated.transpose(1, 2)
...
# ==========================================
# [ 🌀 Fourier Resonant Attention (FRA) 去RoPE化相位調製 ]
# ==========================================
def apply_fra_only(x, head_dim, delta_theta=None):
    B, L, num_heads, D = x.shape

    # 🌊 Pure FRA: 完全移除時域靜態位置編碼 (RoPE)，僅依靠傅立葉諧振調製器的動態相位偏置 delta_theta
...
# ==========================================
# [ 動態修補前向傳播：接合 FRA 相位鎖定 ]
# ==========================================
def make_fra_decoder_forward(layer_idx, fft_phase_lock_module):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        # 🌟 基於物理隔離的模組，利用 FFT 頻域相位動態生成 FRA 的相位偏置量
        delta_theta = fft_phase_lock_module(normed_X, layer_idx)

        # 進行 FRA 相位旋轉 (去 RoPE 化)
        Q = apply_fra_only(Q, self.head_dim, delta_theta=delta_theta).contiguous()

        # K 亦套用對應的平均相位偏移量
        delta_theta_k = delta_theta.mean(dim=2, keepdim=True)
        K = apply_fra_only(K, self.head_dim, delta_theta=delta_theta_k).contiguous()
...
def main():
    print(
        "\n🚀 啟動 GEMMA3 FROM SCRATCH 訓練矩陣 (LoRA + FRA 去RoPE化 1K 混合隨機初始化版，全開放訓練)..."
    )
    ...
    if not os.path.exists(parquet_file):
        raise FileNotFoundError(
            f"找不到實體矩rix {parquet_file}，請先執行 cpt1K.py 產生資料。"
        )
    ...
    print(
        f"\n🔮 正在初始化物理隔離之 Sinc Causal FRA 調製模組 (總參數量精準等於 372,736)..."
    )
    fft_phase_lock = SincCausalRoPE(
        num_layers=26, num_heads=4, head_dim=256, target_params=372736
    )

    print("🛠️ 正在動態修補模型解碼塊 (Gemma3DecoderLayer)，接合去RoPE化 FRA 邏輯...")
    for idx, block in enumerate(base.blocks):
        block.forward = types.MethodType(
            make_fra_decoder_forward(idx, fft_phase_lock), block
        )
    ...
    print("✅ 模型實例化完成，LoRA 與去RoPE化 FRA 調製器已順利掛載。")
    ...
    print(f"🟢 可訓練參數總量 (From Scratch LoRA + FRA): {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_lorafra.pth"),
    )
