...
SAVE_DIR = "./agiv2_cpt_checkpoints_1KTS"
LOG_PATH = "./agiv2_cpt_1kTS_log.csv"

BEST_ROUTER_32K_PATH = "./agiv2_zerogate_ac_checkpoints_32KTS/best_model.pth"
...
def main():
    ...
    # 🔒 永久凍結 gate_fft 與 gate_mem，禁止梯度回傳
    for name, param in model.named_parameters():
        if "gate_fft" in name or "gate_mem" in name:
            param.requires_grad = False
            
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    frozen_gate_params = sum(p.numel() for n, p in model.named_parameters() if "gate_fft" in n or "gate_mem" in n)
    print(f"✅ 設定完成。可訓練參數: {trainable_params:,} | 凍結閘門參數: {frozen_gate_params:,}")
