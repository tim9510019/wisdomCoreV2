...
class HyperDriveByteBuilder:
    def __init__(self):
        print(
            "🚀 啟動量子疊加態資料引擎：AGIV3 (純 Byte 態 | 絕對顯存鎖定 | 動態語義尋路)"
        )
        ...
        self.schema = pa.schema(
            [("text_past", pa.string()), ("text_future", pa.string())]
        )

    def _get_next_document_bytes(self) -> bytes:
        source_type = random.choices(
            list(self.ratios.keys()), weights=list(self.ratios.values()), k=1
        )[0]
        try:
    ...
    def build_and_save(self):
        ...
        with pq.ParquetWriter(output_file, self.schema) as writer:
            pbar = tqdm(
                total=TARGET_SEQUENCES, desc="HyperDrive Writing", smoothing=0.1
            )
            ...
            while seq_count < TARGET_SEQUENCES:
                ...
                calibrated_n = find_text_boundary(
                    current_text, raw_target_n, margin=150
                )
