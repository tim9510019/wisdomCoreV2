# trainCPTAC_AGI_GEMMA3_1K_fra_scratch.py — AGI GEMMA 3 1D Depthwise Conv + Sinc Causal 去RoPE化純波動 From-Scratch 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_fra"
LOG_PATH = "~/agigemma3_scratch_1k_fra_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_fra"
HF_CE_LOSS_THRESHOLD = 9.9  # 從頭訓練初期 Loss 較高，將門檻放寬


# ==========================================
# [ 物理隔離：時域 Sinc Causal 卷積 FRA 模組 ]
# ==========================================
...
def main():
    print(
        "\n🚀 啟動 GEMMA3 FROM SCRATCH 訓練矩陣 (純 FRA 去RoPE化 1K 版本，全開放訓練)..."
    )
    ...
    print(
        f"\n🔮 正在初始化物理隔離之 Sinc Causal FRA 調製模組 (總參數量精準等於 745,472)..."
    )
    ...
    print("✅ 模型實例化完成，去RoPE化純 FRA 調製器已順利掛載。")
    ...
    print(f"🟢 可訓練參數總量 (From Scratch 純 FRA): {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_fra.pth"),
    )
