# trainCPTAC_4KGT_NV.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版 + 原生 Gemma 3 權重起步 + 雙卡 4K)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agiv2_cpt_checkpoints_4K_NV"
LOG_PATH = "~/agiv2_cpt_4k_NV_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1B-CPT_4KGT_NV"
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 訓練矩陣 (雙卡 4K GT 版本 + 原生 Gemma 3 權重起步)...")
    ...
    if last_checkpoint is not None:
        print(f"\n🚀 偵測到 4K GT 時間錨點 {last_checkpoint}，啟動狀態無損恢復...")
        trainer.train(resume_from_checkpoint=last_checkpoint)
    else:
        print("\n🚀 開始全新 4K GT 訓練...")
        trainer.train()
        
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_4kgt.pth"))
