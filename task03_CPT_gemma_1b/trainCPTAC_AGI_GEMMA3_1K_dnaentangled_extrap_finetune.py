...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_dnaentangled_extrap"
LOG_PATH = "~/agigemma3_scratch_1k_dnaentangled_extrap_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_dnaentangled_extrap"
...
class DNAGatedSincCausalRoPE(nn.Module):
    def __init__(self, num_layers=26, target_params=745472, W=64):
        """
        DNA_GateDual / DNA_Entangled 專用雙螺旋鹼基對門控調製模組。
...
def make_dna_entangled_decoupled_decoder_forward(layer_idx, gated_module, alpha=0.1):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        delta_theta, gate = gated_module(X_part_aggr, layer_idx) # (B, L, 1, 128), (B, L, 8)
...
def main():
    print(f"🔮 初始化 GEMMA 3 1B DNA_Entangled (雙螺旋波粒因果糾纏) From-Scratch 訓練引擎...")
    ...
    # 實例化 DNA_Entangled 雙螺旋因果糾纏模組 (等參量鎖定 params: 745,472)
    ...
    # 📂 載入 DNA_Entangled 最佳預訓練權重
    preload_path = os.path.expanduser("~/agigemma3_scratch_checkpoints_1K_dnaentangled/best_cpt_model.pth")
    if os.path.exists(preload_path):
        print(f"📂 [Finetune] 載入 DNA_Entangled 最佳預訓練權重: {preload_path}")
        sd = torch.load(preload_path, map_location="cpu")
        model.load_state_dict(sd, strict=True)
    else:
        print(f"❌ 警告: 找不到預訓練權重 {preload_path}，將從隨機初始化開始！")
        
    for param in model.parameters():
        param.requires_grad = True
            
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"🟢 可訓練參數總量 (From Scratch DNA_Entangled): {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_dnaentangled.pth"),
    )
