# trainCPTAC_AGI_GEMMA3_1K_headdual_scratch.py — AGI GEMMA 3 頭級子空間正交解耦 HEAD_DUAL From-Scratch 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_headdual"
LOG_PATH = "~/agigemma3_scratch_1k_headdual_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_headdual"
...
# ==========================================
# [ 物理隔離：時域 Sinc Causal 卷積模組 (1個 Wave 頭調製器, params: 745,472) ]
# ==========================================
class SincCausalRoPE(nn.Module):
    def __init__(self, num_layers=26, target_params=745472, W=64):
        """
        頭級解耦專用的波動頭調製器。
        總參數量: 745,472，每層配額 28,672。
        重塑為 224 x 128 (輸入 224 特徵通道，投射至 128 相位維度，對應 1 個 Wave Head)。
        """
        super().__init__()
    ...
    def forward(self, x, layer_idx):
        ...
        w_layer = self.weight[start_idx:end_idx].view(224, 128)
        
        with torch.autocast(device_type="cuda", enabled=False):
            x_slice = x[..., :224].to(torch.float32)
...
def make_head_decoupled_decoder_forward(layer_idx, fft_phase_lock_module):
    def forward(self, X, shift_size=0, n_split_index=None):
        ...
        # 1. 為了進行頭級隔離，手動將 Key 和 Value 展開對齊 Query 的 4 個頭
...
def main():
    print(f"🔮 初始化 GEMMA 3 1B HEAD_DUAL From-Scratch 訓練引擎...")
    ...
    # 實例化頭級子空間正交解耦調製器 (params: 745,472, weight shape: 224 x 128)
    ...
    print(f"🟢 可訓練參數總量 (From Scratch HEAD_DUAL): {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_headdual.pth"),
    )
