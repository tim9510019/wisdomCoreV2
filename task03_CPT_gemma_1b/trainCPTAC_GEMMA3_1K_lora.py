# trainCPTAC_GEMMA3_1K_lora.py — GEMMA 3 LoRA 微調預訓練引擎 (GEMM 全凍結，745,472 可訓練參數)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/gemma3_cpt_checkpoints_1K_lora"
LOG_PATH = "~/gemma3_cpt_1k_lora_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/GEMMA3-1B-CPT_1K_lora"
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 CPT 訓練矩陣 (LoRA 微調 1K 版本，GEMM 全凍結)...")
    ...
    model = AGIV2GForCausalLMT(base, use_gc=True, gate_entropy_lambda=GATE_ENTROONT_LAMBDA if 'GATE_ENTROONT_LAMBDA' in globals() else 0.01)
    print("✅ 模型實例化完成，並成功接合官方預訓練特徵。")
    
    # 🔒 鎖定非 LoRA 參數，僅開放 LoRA 參數進行更新
    print("\n🔒 鎖定全局預訓練參數，僅開放 LoRA 參數更新...")
    ...
    for name, param in model.named_parameters():
        if "lora_" in name:
            ...
            param.data = param.data.to(torch.float32) # LoRA 權重使用 fp32 進行穩定更新
        
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    frozen_params = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    print(f"✅ 設定完成！")
    print(f"🔒 凍結參數總量: {frozen_params:,}")
    print(f"🟢 可訓練參數總量 (LoRA): {trainable_params:,} (預期應精準等於 745,472)")
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_gemma3_cpt_1k_lora.pth"))
