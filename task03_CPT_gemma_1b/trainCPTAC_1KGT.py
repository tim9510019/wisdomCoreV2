# trainCPTAC_1KGT.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版 + 原生 Gemma 3 權重起步)
import os
import sys
import csv
import time
...
SAVE_DIR = "./agiv2_cpt_checkpoints_1KGT"
LOG_PATH = "./agiv2_cpt_1kGT_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1300M-blackwell-CPT_1KGT"
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 第一階訓練矩陣 (原生 Gemma 3 權重起步 + 1K GT)...")
    ...
    if last_checkpoint is not None:
        print(f"\n🚀 偵測到 1K GT 時間錨點 {last_checkpoint}，啟動狀態無損恢復...")
        trainer.train(resume_from_checkpoint=last_checkpoint)
    else:
        print("\n🚀 開始全新 1K GT 訓練...")
        trainer.train()
        
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_1kgt.pth"))
