...
def main():
    ...
    model_1k_path = "./agiv2_zerogate_ac_checkpoints_1K/best_model.pth"
    if os.path.exists(model_1k_path):
        print(f"\n🔄 成功尋獲 1K 階段之最佳權重，正在載入以進行 2K 延伸訓練: {model_1k_path}")
        state_dict = torch.load(model_1k_path, map_location="cpu")
        model.load_state_dict(state_dict, strict=False)
    else:
        print(f"\n⚠️ 找不到 1K 權重檔案 {model_1k_path}，將從頭開始訓練！")
