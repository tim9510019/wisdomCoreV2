...
class SFTDataset(Dataset):
    def __init__(self, tokenizer, split="train", max_length=32768, max_samples=None):
        ...
        data = load_dataset("cerebras/Synth-Long-SFT32K", split="train_narrativeqa_aug_32k")
        ...
        for ex in data:
            ...
            user_msg = conversations[0].get("value", conversations[0].get("content", ""))
            model_msg = conversations[1].get("value", conversations[1].get("content", ""))
            
            # 略過空值
            if not user_msg or not model_msg: continue
            
            # 使用 Tokenizer 產生 User 提問的長度 (此部分不計算梯度)
            user_chat = [{"role": "user", "content": user_msg}]
            user_prompt = tokenizer.apply_chat_template(user_chat, tokenize=False, add_generation_prompt=True)
            user_ids = tokenizer(user_prompt, add_special_tokens=False).input_ids
            
            # 使用 Tokenizer 產生完整的一輪對話
            full_chat = [{"role": "user", "content": user_msg}, {"role": "model", "content": model_msg}]
