# trainCPTAC_AGI_GEMMA3_1K_fra.py — AGI GEMMA 3 1D Depthwise Conv + Sinc Causal 去RoPE化純波動預訓練引擎 (GEMM 全凍結，745,472 可訓練參數，物理隔離)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_cpt_checkpoints_1K_fra"
LOG_PATH = "~/agigemma3_cpt_1k_fra_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-CPT_1K_fra"
...
def apply_fra_only(x, head_dim, delta_theta=None):
    ...
    # 🌊 Pure FRA: 完全移除時域靜態位置編碼 (RoPE)
    # 僅依靠傅立葉諧振調製器的動態相位偏置 delta_theta
    if delta_theta is not None:
        emb = delta_theta # shape: (B, L, num_heads, D // 2)
    else:
        return x
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 CPT 訓練矩陣 (純 FRA 去RoPE化 1K 版本，GEMM 全凍結)...")
    ...
    # 實例化物理隔離的 Sinc Causal 調製模組 (All FRA Version: 745472 params)
    ...
    # 動態修補 (Monkey Patch) 所有 Block 的 forward 以引入去RoPE化 FRA 相位鎖定調製
    print("🛠& 正在動態修補模型解碼塊 (Gemma3DecoderLayer)，接合去RoPE化 FRA 邏輯...")
    for idx, block in enumerate(base.blocks):
        block.forward = types.MethodType(
            make_fra_decoder_forward(idx, fft_phase_lock),
            block
        )
    ...
    # 🔒 鎖定全局預訓練參數，僅開放實體隔離之 FRA 參數更新
    print("\n🔒 鎖定全局預訓練參數，僅開放 FRA 參數更新...")
    ...
    print(f"🟢 可訓練參數總量 (All FRA): {trainable_params:,} (預期應精準等於 745,472)")
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agigemma3_cpt_1k_fra.pth"))
