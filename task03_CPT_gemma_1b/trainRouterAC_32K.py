...
SAVE_DIR = "./agiv2_zerogate_ac_checkpoints_32K"
LOG_PATH = "./zerogate_32k_ac_32k_log.csv"
...
def main():
    ...
    # 🌟 載入 16K 階段的 best_model 權重
    if os.path.exists(BEST_16K_PATH):
        print(
            f"\n🔄 成功尋獲 16K 階段之最佳權重，正在載入以進行 32K 延伸訓練: {BEST_16K_PATH}"
        )
        state_dict = torch.load(BEST_16K_PATH, map_location="cpu")
        model.load_state_dict(state_dict, strict=False)
    else:
        print(f"\n⚠️ 找不到 16K 權重檔案 {BEST_16K_PATH}，將從頭開始訓練！")
