...
SAVE_DIR = "./agiv2_zerogate_ac_checkpoints_128K"
LOG_PATH = "./zerogate_32k_ac_128k_log.csv"
BEST_64K_PATH = "./agiv2_zerogate_ac_checkpoints_64K/best_model.pth"
...
# 🚀 模型與資料參數
TARGET_LENGTHS = [131072]  # 訓練資料基準長度
...
def main():
    ...
    # 🌟 載入 64K 階段的 best_model 權重
    if os.path.exists(BEST_64K_PATH):
        print(
            f"\n🔄 成功尋獲 64K 階段之最佳權重，正在載入以進行 128K 延伸訓練: {BEST_64K_PATH}"
        )
        state_dict = torch.load(BEST_64K_PATH, map_location="cpu")
        model.load_state_dict(state_dict, strict=False)
    else:
        print(f"\n⚠️ 找不到 64K 權重檔案 {BEST_64K_PATH}，將從頭開始訓練！")
