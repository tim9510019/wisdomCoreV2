...
from utils import (
    AGIV2GForCausalLM,
    transplant_and_freeze,
    QuantumRouterEngineAC,
    DynamicACDataset,
)
...
SAVE_DIR = "./agiv2_zerogate_ac_checkpoints_8K"
LOG_PATH = "./zerogate_32k_ac_8k_log.csv"
BEST_4K_PATH = "./agiv2_zerogate_ac_checkpoints_4K/best_model.pth"

# 🚀 訓練步數與排程 (使用者自訂)
MAX_STEPS = 1000  # 🏆 指定要訓練的總步數 (取代 num_train_epochs)
WARMUP_STEPS = 200  # 🌟 改用 WARMUP_STEPS 避免 deprecated 警告 (約 20%)
...
class ACDataCollator:
    ...
    def __call__(self, features):
        ...
        for f in features:
            f["pos_ids"] = [self.pad_token_id] * (max_pos - len(f["pos_ids"])) + list(
                f["pos_ids"]
            )
            f["neg_ids"] = [self.pad_token_id] * (max_neg - len(f["neg_ids"])) + list(
                f["neg_ids"]
            )
...
class RouterTrainerAC(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        for b in core_model.blocks:
            if hasattr(b, "gate_mem"):
                # 🌟 將此處的 20.0 降低為 15.0 以適應訓練
                gate_penalty = (
                    gate_penalty + torch.exp(-torch.abs(b.gate_mem) * 15.0).mean()
                )
...
def main():
    ...
    # 🌟 載入 4K 階段的 best_model 權重
    if os.path.exists(BEST_4K_PATH):
        print(
            f"\n🔄 成功尋獲 4K 階段之最佳權重，正在載入以進行 8K 延伸訓練: {BEST_4K_PATH}"
        )
        state_dict = torch.load(BEST_4K_PATH, map_location="cpu")
        model.load_state_dict(state_dict, strict=False)
    else:
        print(f"\n⚠️ 找不到 4K 權重檔案 {BEST_4K_PATH}，將從頭開始訓練！")
    ...
    trainer = RouterTrainerAC(
        ...
        data_collator=ACDataCollator(
            pad_id
        ),  # 🌟 修正 ValueError 的關鍵，提供自訂的 DataCollator 加入 Left Padding
