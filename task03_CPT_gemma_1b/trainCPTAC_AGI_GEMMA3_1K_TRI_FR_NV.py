# trainCPTAC_AGI_GEMMA3_1K_TRI_FR_NV.py — AGI GEMMA 3 (正三角形幾何約束 + 非對稱凍結版)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_cpt_checkpoints_1K_TRI_FR_NV"
LOG_PATH = "~/agigemma3_cpt_1k_TRI_FR_NV_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-CPT_1K_TRI_FR_NV"
...
def main():
    print(
        f"\n🚀 啟動 AGIGEMMA3 CPT 訓練矩陣 (前 {N_FFT_F} 後 {N_FFT_B} 層純 FFT + 正三角形幾何約束 + 非對稱參數凍結版)..."
    )
    ...
    print("\n🔄 正在掛載原生 google/gemma-3-1b-it 權重並預設凍結...")
    ...
    # 🌟 核心調整：實施「非對稱參數凍結」，僅解凍 FFT 模組與三頭幾何投影器
    print("\n🔧 啟動非對稱凍結設定 (僅訓練 FFT 區塊與幾何投影頭):")
    for name, param in model.named_parameters():
        if "tri_head" in name or "pre_fft_blocks" in name or "post_fft_blocks" in name:
    ...
    print(f"✅ 設定完成。可訓練參數總量 (FFT + Tri-Head): {trainable_params:,}")
    print(f"🔒 凍結參數總量 (Gemma 3 主幹): {frozen_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_cpt_1k_tri_fr.pth"),
    )
