...
def generate_cpt_autoregressive(model, tokenizer, prompt_ids, expected_b_size, max_display, temperature, repetition_penalty):
    ...
    print(f"✨ [參數] Gen_Temperature={temperature}, Repetition_Penalty={repetition_penalty}")
        ...
        if step == 0:
            ...
            for i, block in enumerate(core.blocks):
                if hasattr(block, 'avg_g_loc'):
                    ...
                    print(f"  ▶ Block {i:02d} | Loc: {loc:.4f} | Mem: {mem:.4f} | FFT: {fft:.4f}  [主導: {dom}]")
            print("-" * 50)
...
if __name__ == "__main__":
    ...
    if len(prompt_ids) > MAX_PROMPT_LENGTH:
        print(f"✂️ [注意] 原始上下文 ({len(prompt_ids)}) 超載，自動截斷保留後 {MAX_PROMPT_LENGTH} 個 Token。")
