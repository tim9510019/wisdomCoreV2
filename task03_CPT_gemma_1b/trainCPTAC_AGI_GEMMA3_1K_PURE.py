# trainCPTAC_AGI_GEMMA3_1K_PURE.py — AGI GEMMA 3 (前N後N純FFT) PURE 模式對照實驗
import os
import sys
import csv
import time
...
# 🌟 N_FFT 可以調整，是自然數，代表在前 N 層與後 N 層使用純 FFT 相位鎖定
N_FFT_F = 6
N_FFT_B = 4

MODEL_ID = "google/gemma-3-1b-it"
DATASET_DIR = "~/agiv2_stage1_1K"
SAVE_DIR = "~/agigemma3_cpt_checkpoints_1K_PURE"
LOG_PATH = "~/agigemma3_cpt_1k_PURE_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-CPT_1K_PURE"
...
def apply_lora_to_model(base_model, r=8, alpha=16):
    ...
    總參數增加量: 26 * r * (1152 + 1024 + 1152 + 256) = 26 * r * 3584 個參數。
    """
...
# ==========================================
# 專屬 AGIGEMMA3 的 CausalLM Wrapper
# 支援 pre_fft_blocks, blocks, post_fft_blocks 和 gradient checkpointing
# ==========================================
class AGIGEMMA3ForCausalLM(nn.Module):
...
def main():
    print(
        f"\n🚀 啟動 AGIGEMMA3 CPT 訓練矩陣 [PURE 模式] (前 {N_FFT_F} 後 {N_FFT_B} 層 PureFFT + Gemma3 權重凍結)..."
    )
    ...
    base = AGIGEMMA3(
        ...
        fft_block_type="pure",  # 🌟 PURE 模式
    )

    print("\n🔄 正在掛載原生 google/gemma-3-1b-it 權重...")
    base = transplant_and_freeze(MODEL_ID, base)

    print(f"🌸 正在注入 LoRA 微調模組 (r={LORA_R}, alpha={LORA_ALPHA})...")
    base = apply_lora_to_model(base, r=LORA_R, alpha=LORA_ALPHA)

    # 使用專屬 Wrapper
    model = AGIGEMMA3ForCausalLM(base, use_gc=True)
    print("✅ 模型實例化完成，並成功接合官方預訓練特徵。")

    # 🔒 Gemma3 主幹凍結，只訓練 FFT blocks 與 LoRA 參數
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    frozen_params = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    print(f"🔒 Gemma3 凍結參數: {frozen_params:,} | 🟢 FFT + LoRA 可訓練參數: {trainable_params:,}")
    ...
    torch.save(
        ...
        os.path.join(SAVE_DIR, "final_agigemma3_cpt_1k.pth"),
    )
