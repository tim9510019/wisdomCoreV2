# trainCPTAC_GEMMA3_1K_lora_scratch.py — GEMMA 3 LoRA From-Scratch 訓練引擎 (745,472 LoRA + 1.2B Base, 全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/gemma3_scratch_checkpoints_1K_lora"
LOG_PATH = "~/gemma3_scratch_1k_lora_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/GEMMA3-1B-SCRATCH_1K_lora"
HF_CE_LOSS_THRESHOLD = 9.9          # 從頭訓練初期 Loss 較高，將門檻放寬以觸發上載驗證
...
def apply_lora_to_model(base_model, r=8, alpha=16):
    """
    對 model 中的所有 26 個解碼塊的 W_q_loc 與 W_v_loc 進行 LoRA 注入。
    總參數增加量: 26 * 8 * (1152 + 1024 + 1152 + 256) = 745,472 個參數。
    """
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 FROM SCRATCH 訓練矩陣 (LoRA 組，全部參數皆可訓練)...")
    ...
    # 注入 LoRA
    print("\n💉 正在注入 LoRA 微調模組 (r=8, alpha=16) 至 W_q_loc 與 W_v_loc...")
    base = apply_lora_to_model(base, r=8, alpha=16)
    
    model = AGIV2GForCausalLMT(base, use_gc=True, gate_entropy_lambda=GATE_ENTROPY_LAMBDA if 'GATE_ENTROPY_LAMBDA' in globals() else 0.01)
    
    # 🟢 FROM SCRATCH: 讓所有參數（包含隨機初始化的 Base 與 LoRA 參數）均參與訓練
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_gemma3_scratch_1k_lora.pth"))
