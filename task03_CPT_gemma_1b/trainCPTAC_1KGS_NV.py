# trainCPTAC_1KGS_NV.py — AGIV2 持續預訓練終極引擎 (動態 MoE 路由版 + 原生 Gemma 3 權重起步)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agiv2_cpt_checkpoints_1KGS_NV"
LOG_PATH = "~/agiv2_cpt_1KGS_log_NV.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIV2-1B-CPT_1KGS_NV"
    ...
    def __init__(self, path=LOG_PATH, save_dir=SAVE_DIR):
        ...
        if os.path.exists(self.path):
            ...
            with open(self.path, 'w', newline='') as f:
                csv.writer(f).writerow(['step', 'loss', 'ce_loss', 'gate_ent_loss', 'eval_loss', 'g_loc', 'g_mem', 'time'])
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 第一階訓練矩陣 (原生 Gemma 3 權重起步 + 1K GS)...")
    ...
    # 🌟 從原生 Gemma 3 權重起步，載入預訓練參數並初始化 AGIV2 的新結構
    ...
    if last_checkpoint is not None:
        print(f"\n🚀 偵測到 1K GS 時間錨點 {last_checkpoint}，啟動狀態無損恢復...")
        trainer.train(resume_from_checkpoint=last_checkpoint)
    else:
        print("\n🚀 開始全新 1K GS 訓練...")
        trainer.train()
        
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_1kgs.pth"))
