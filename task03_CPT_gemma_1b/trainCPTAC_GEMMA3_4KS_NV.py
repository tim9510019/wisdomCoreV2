# trainCPTAC_GEMMA3_4KS_NV.py — GEMMA 3 原始架構預訓練引擎 (4K 雙卡 S 系列延續訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/gemma3_cpt_checkpoints_4KS_NV"
LOG_PATH = "~/gemma3_cpt_4KS_NV_log.csv"
BEST_1KS_NV_PATH = "~/gemma3_cpt_checkpoints_1KS_NV/best_cpt_model.pth"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/GEMMA3-1B-CPT_4KS_NV"
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 CPT 第二階訓練矩陣 (純淨架構 4K 雙卡版)...")
    ...
    if os.path.exists(BEST_1KS_NV_PATH):
        ...
        print(f"\n⚠️ 找不到 1KS NV 權重檔案 {BEST_1KS_NV_PATH}，將完全從零開始訓練！")
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_gemma3_cpt_4ks.pth"))
