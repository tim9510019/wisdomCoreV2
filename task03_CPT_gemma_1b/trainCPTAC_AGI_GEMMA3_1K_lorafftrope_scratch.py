# trainCPTAC_AGI_GEMMA3_1K_lorafftrope_scratch.py — AGI GEMMA 3 LoRA + Sinc Causal From-Scratch 訓練引擎 (全開放訓練)
import os
import sys
import csv
import time
...
SAVE_DIR = "~/agigemma3_scratch_checkpoints_1K_lorafftrope"
LOG_PATH = "~/agigemma3_scratch_1k_lorafftrope_log.csv"
...
# HuggingFace 自動上傳配置
REPO_ID = "tim9510019/AGIGEMMA3-1B-SCRATCH_1K_lorafftrope"
...
# ==========================================
def main():
    print("\n🚀 啟動 GEMMA3 FROM SCRATCH 訓練矩陣 (LoRA + Sinc Causal 雙效混合，全開放訓練)...")
    ...
    # 💉 注入 LoRA (r=4, alpha=8) 至 W_q_loc 與 W_v_loc (佔 372,736 參數)
    print("\n💉 正在注入 LoRA 微調模組 (r=4, alpha=8) 至 W_q_loc 與 W_v_loc...")
    base = apply_lora_to_model(base, r=4, alpha=8)
    
    # 🔮 實例化物理隔離的 Sinc Causal 調製模組 (56 特徵切片，佔 372,736 參數)
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agigemma3_scratch_1k_lorafftrope.pth"))
