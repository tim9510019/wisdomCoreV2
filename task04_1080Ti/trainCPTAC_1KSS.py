...
# 建議 1080Ti 單卡通常為 "0"，若你的系統確實掛在 "1" 請保持原樣
...
SAVE_DIR = "./agiv2_cpt_checkpoints_1KST"
LOG_PATH = "./agiv2_cpt_1kST_log.csv"
...
SAVE_TOTAL_LIMIT = 2               
LOGGING_STEPS = 1                  
# ⚠️ [修改] 針對 11GB VRAM 極限微調
BATCH_SIZE_PER_DEVICE = 1          # 從 2 降至 1
GRAD_ACCUMULATION_STEPS = 32       # 從 16 升至 32 以保持等效 Batch Size
LEARNING_RATE = 1e-4               
CHUNK = 256
...
# ==========================================
def main():
    print("\n🚀 啟動 AGIV2 CPT 第一階訓練矩陣 (降級適配 1080Ti 模式)...")
    ...
    # ⚠️ [修改] 降級為 float16，1080Ti 不支援 bfloat16
    ...
    args = TrainingArguments(
        ...
        # ⚠️ 注意：若 paged_adamw_8bit 在 1080Ti 報錯，請改為 "adamw_torch"
    ...
    torch.save(model.state_dict() if not hasattr(model, 'module') else model.module.state_dict(), os.path.join(SAVE_DIR, "final_agiv2_cpt_1k.pth"))
